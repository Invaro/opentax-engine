...
| **Calculation proofs and citations** | Yes. Every rule carries a statutory / form citation with a verbatim excerpt; every computing response carries `corpusMerkleRoot` and `artifactHash`, and with `includeProof: true` the computing tools (`calculate_tax`, `compute_return`, `calculate_business_tax`, `calculate_fiduciary_tax`, `determine_dependent`) return the full proof artifact (every applied rule, every input, every assumption, every rounding, ~200 KB) that verifies offline against the corpus Merkle root, and the format is specified so a third party can write an independent checker. `explain_rule` returns any rule's formula and law text. | `proof`, `corpusMerkleRoot`, [PROOF-FORMAT.md](../PROOF-FORMAT.md), `explain_rule` |
